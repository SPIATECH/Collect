...
import {getTagList, getTagListByTagId, deleteTagList, getTagType, selectTagList, createUnGroup } from '../../../redux'
import { 
    NoDeviceTagInfoMessage, 
    TableNameColumnHeader, 
    DeviceTagListCountperPage, 
    EnterKeyCode,
    AddTagButtonLabel,
    DeleteTagWarningHeading, 
...
function TagList(props){
    ...
    dispatch(createUnGroup())
    //const [keyRowSelect, setKeyRowSelect] = React.useState(false);
    const [selectedDeviceTypeName, setSelectedDeviceTypeName] = React.useState(false);
    const [selectedDeviceTypeDisplayName, setSelectedDeviceTypeDisplayName] = React.useState(false);
    const [selectedDeviceTypeId, setSelectedDeviceTypeId] = React.useState(false);
    
    const [selectedDeviceName, setSelectedDeviceName] = React.useState(false);
    ...
    useEffect(() => {
        ...
        if(hash.length > 1){
            ...
            if (paramItems['device'] && paramItems['edit']) {
                setSelectedDeviceTypeId(paramItems['device'])
                setSelectedDeviceId(paramItems['edit'])
                dispatch(getTagType(paramItems['device']))
                dispatch(getTagList(paramItems['edit']))
            } 
        }
       
    }, []);
   
    const deleteButtonClicked = (tagId) => {
        console.log(tagId)
        if(tagId != ""){
            isConfirm(true);
    ...
    function deleteFile(){
        var sedeviceID = selectedDeviceId.toLowerCase()
        dispatch(deleteTagList(tagDeleteID, sedeviceID))
        isConfirm(false);
    }
            ...
            <DialogContent>
                ...
                <Row className="show-grid">
                    ...
                    <div className="delepopContent">
                        ...
                        <h6>{DeleteTagWarningHeading}</h6>
                        <p>{DeleteTagWarningMessage}</p>
                    </div>
                    </Col>
                    <Col xs={12} md={12}>
    ...
    function setTableHeader(item){
        if(props.TagTypeBucket.length > 0){
            var deviceTypeProp = props.TagTypeBucket[0].properties
            var headerName = true
    ...
    function addTag() {
        window.location = window.location.origin+"/#/datasources/devices/tags/addTags?device="+selectedDeviceTypeId+"&edit="+selectedDeviceId
    }

    function onRowSelect(tagName, tagId) {
        dispatch(selectTagList(tagName))
        dispatch(getTagListByTagId(tagId));
        window.location = window.location.origin+"/#/datasources/devices/tags/editTags?device="+selectedDeviceTypeId+"&edit="+tagId
    }
    ...
    function actionFormatter(cell, row) {
        console.log(selectedDeviceTypeDisplayName);
        return <div><Tooltip title={DeleteLabel} placement="top" arrow><span id={cell}><DeleteOutlineOutlinedIcon onClick={() => deleteButtonClicked(cell)} /></span></Tooltip></div>;
    }

    const customStyle = (cell, row) => {
        //setKeyRowSelect(row.name)
        //setIdRowSelect(row.id)
        //onRowSelect(row, true, null)
        return {
    ...
    const options = {
        sizePerPageList: [
            { 
                text: DeviceTagListCountperPage.default, 
                value: DeviceTagListCountperPage.default
            }, 
            {
                text: DeviceTagListCountperPage.option_1, 
                value:DeviceTagListCountperPage.option_1
            }, 
            {
                text: DeviceTagListCountperPage.option_2,
                 value: DeviceTagListCountperPage.option_2
            }],
        sizePerPage: DeviceTagListCountperPage.default
    } 
    ...
    if (props.TagListBucket != undefined && props.TagListBucket.length > 0) {

        var additionalProp = "";
        for(var i = 0; i < props.TagListBucket.length; i++){
            additionalProp = props.TagListBucket[i]
                            ...
                            <div className="deviceListHead">
                                <div className="add-common-btn pos-abs">
                                    <Button 
                                        ...
                                        <AddOutlinedIcon color="primary" /> {AddTagButtonLabel}
                                    </Button>
                                </div>
                                <div className="section-tags section-device-list">
                                    <BootstrapTable data={props.TagListBucket} options={options} bordered={false} pagination hover keyBoardNav={keyBoardNavProp}>

...
import './Devices.scss'
import {getDeviceTypeById, getDevices, addDevice } from '../../../redux'
...
class addDevices extends React.Component {
    ...
    componentDidMount() {
        ...
        if(hash.length > 1){
            ...
            if (paramItems['device']) {
                ...
                this.props.dispatch(getDevices(paramItems['device']))
            } 
        }     
    }
    ...
    handleSubmit = (e) => {
        ...
        if(hasError){
            ...
            this.props.dispatch(addDevice(formData))
            window.location = window.location.origin+"/#/datasources/devices?device="+this.state.deviceTypeId
        }
        e.preventDefault();
    }
    ...
    handleDeviceNameChange = (e) =>{
        console.log(this.props.DevicesBucket)
    ...
    handleSetStateValues(fieldName){
        if(this.props.selectedDeviceBucket.length > 0){
            var formProps = this.props.selectedDeviceBucket[0].additionalProp1
...
export default connect(mapStateToProps)(withStyles(formStyle)(addDevices))
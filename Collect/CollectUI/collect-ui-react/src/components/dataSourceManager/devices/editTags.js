...
import './Devices.scss'
import {removeSelectedTagList, updateTagList, getTagType, getTagList } from '../../../redux'
...
const mapStateToProps = store => {
    return {
        selectedTagListBucket: store.tagList.selectedTagListBucket,
        selectedTagList: store.tagList.selectedTagList,
...
class editTags extends React.Component {
    ...
    componentDidMount() {
        ...
        if(hash.length > 1){
            ...
            if (paramItems['device']&& paramItems['edit']) {
                this.props.dispatch(removeSelectedTagList())
                this.setState({tagName:''})
                this.setState({tagOldName:''})
    ...
    handleTagSubmit = (e) => {
        ...
        if(hasError){
            ...
            var tagUpdateId = this.props.selectedTagListBucket[0].id
            var backdeviceId = this.props.selectedTagListBucket[0].deviceId
            delete obj.tagName
            ...
            if(tagUpdateId !== "" && tagUpdateId !== false){
                this.props.dispatch(updateTagList(tagUpdateId, formData))
            }
            window.location = window.location.origin+"/#/datasources/devices/tags?device="+this.state.deviceTypeId+"&edit="+backdeviceId
        }
        e.preventDefault();
    }
    ...
    handleTagNameChange = (e) =>{
        ...
        this.setState({tagOldName:this.props.selectedTagListBucket[0].name})
        var tagExist = 1
       
        if(this.state.tagOldName !== value){
        ...
        if(this.state.isError && tagExist === 1){
    ...
    handleSetStateValues(fieldName){
        console.log(this.props.selectedTagListBucket);
        console.log('this.props.selectedTagListBucket');
        if(this.props.selectedTagListBucket.length > 0){
            var formProps = this.props.selectedTagListBucket[0]
            ...
            if(this.state.tagName === "" && this.state.tagOldName === ""){
                if("name" in formProps){
                    this.setState({tagName:formProps["name"]})
                    this.setState({tagOldName:formProps["name"]})
                }
            }
...
export default connect(mapStateToProps)(withStyles(formStyle)(editTags))
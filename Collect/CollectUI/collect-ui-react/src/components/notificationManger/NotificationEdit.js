...
function NotificationEdit(props) {
    ...
    const handleSubmit = (datas, updateId) => {
        
        if (updateId != 0) {
          dispatch(updateNotification(updateId, datas))
          dispatch(removeNotificationInfo())
          dispatch(isEditNotification(false));
        } else {
          dispatch(addNotification(datas))
        }
        window.location = window.location.origin+"/#/notifications";
        //props.onClose(true)
        //dispatch(getNotification())
    }
   
    const  handleDialogBack = (event) => {
        isSetSelectAlert(false);
    }
    ...
    function onSelectedAlert(alertID, alertName, tagID) {
        ...
        if(dynamicHash.length > 1){
            ...
            if (hashItems['notification']) {
                ...
                dispatch(getNotificationById(hashItems['notification'], false))
            } 
        }
    }
    ...
    if (props.TagAlertsBucket.length > 0 && props.SingleNotificationBucket.length > 0) {
        if(isSelectAlert){
            var showAlertDiv = 'close-div'
            var showFormDiv = 'open-div'
        }else{
             var showFormDiv = 'close-div'
             var showAlertDiv = 'open-div'
        }
                ...
                <div className="tabContentarea">
                    <Grid item xs={12}>
                        <Paper className={classes.paper}>
                            <div className="section-alerts" id={showAlertDiv}>
                                <div className="selected-row-alert" style={{ display: props.SelectedAlertsBucket != undefined && props.SelectedAlertsBucket.length > 0 ? 'block':'none' }}>
                                    <label>{SelectedAlertHeading}</label>
                                    {renderSelectedAlert(props)}
                                </div>
                                {renderItem(props)}
                            </div>
                            <div className="notifi-add-form" id={showFormDiv} >
                                <NotificationEditForm onSubmit={handleSubmit} onClick={handleDialogBack}  onClose={handleDialogClose}/>
                            </div>
                        </Paper>
                    </Grid>
                </div>
...
export default connect(mapStateToProps)(NotificationEdit)